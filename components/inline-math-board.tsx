...
export function InlineMathBoard() {
  ...
  const [strokeColor, setStrokeColor] = React.useState<string>('black')
  ...
  React.useEffect(() => {
    // Toggle erase mode on the canvas whenever the eraser state changes
  ...
  const onDone = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/create-inline-math', {
        method: 'POST',
      ...
      // Normalize: strip $...$, $$...$$, \( ... \), \[ ... \], or span wrappers
      const extractFromSpan = () => {
      ...
      const fromSpan = extractFromSpan()
      if (fromSpan) {
        latex = fromSpan
      } else {
      ...
      // Insert inline math node via HTML representation to avoid adding $ wrappers back
      const escapeAttr = (s: string) =>
        s.replace(/&/g, '&amp;').replace(/"/g, '&quot;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
      const html = `<span data-type="inline-math" data-latex="${escapeAttr(latex)}"></span>`
      ...
      <PopoverContent className="w-120 mb-2 p-3 rounded-xl">
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <h4 className="leading-none font-medium">Draw Inline Equation</h4>
